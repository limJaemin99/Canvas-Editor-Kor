...
export class CommandAdapt {
  ...
  public title(payload: TitleLevel | null) {
    ...
    // 제목/편/장/절/관/부칙은 가운데 정렬이 필요한 레벨
    const centerAlignLevels = [
      TitleLevel.TITLE,
      TitleLevel.PYEON,
      TitleLevel.JANG,
      TitleLevel.BUCHIK,
    ...
    changeElementList.forEach(el => {
      if (payload) {
        // ZERO(\n) 요소: 가운데 정렬 레벨이면 rowFlex만 설정, 텍스트 스타일은 건너뜀
        if (!el.type && el.value === ZERO) {
          if (centerAlignLevels.includes(payload)) {
            el.rowFlex = RowFlex.CENTER
            delete el.indent
          } else if (payload === TitleLevel.JO || payload === TitleLevel.BYEOLJI) {
            delete el.rowFlex
            delete el.indent
          }
          return
        ...
        // 조/별지/별표: 굵게만 적용, 크기는 현재 값 그대로 유지
        // (size를 명시적으로 저장해야 JSON 저장→불러오기 시 titleSizeMapping으로 덮어씌워지지 않음)
        else if (payload === TitleLevel.JO || payload === TitleLevel.BYEOLJI) {
          if (isTextLikeElement(el)) {
            el.bold = true
            // 현재 size가 없으면 defaultSize로 명시 저장 (없으면 불러오기 시 JO size=16으로 강제됨)
            if (!el.size) {
              el.size = this.draw.getOptions().defaultSize
            }
          }
          delete el.rowFlex
          delete el.indent
        ...
        // 제목 해제
        if (el.value === ZERO) {
          // ZERO 요소에 titleId가 없어도 rowFlex 정리
          delete el.rowFlex
          delete el.indent
          return
        }
    ...
    // changeElementList에 포함되지 않은 단락 시작 ZERO 요소도 처리
    // (elementList에서 changeElementList의 첫 요소 앞에 있는 ZERO를 찾아 rowFlex 설정)
    if (changeElementList.length > 0) {
      const firstEl = changeElementList[0]
      const firstElIndex = elementList.indexOf(firstEl)
      if (firstElIndex > 0) {
        const prevEl = elementList[firstElIndex - 1]
        if (prevEl && !prevEl.type && prevEl.value === ZERO) {
          if (payload && centerAlignLevels.includes(payload)) {
            prevEl.rowFlex = RowFlex.CENTER
            delete prevEl.indent
          } else if (!payload) {
            delete prevEl.rowFlex
            delete prevEl.indent
          } else if (payload === TitleLevel.JO || payload === TitleLevel.BYEOLJI) {
            delete prevEl.rowFlex
            delete prevEl.indent
  ...
  public indentIncrease() {
    const isDisabled = this.draw.isReadonly() || this.draw.isDisabled()
    if (isDisabled) return
    const { startIndex, endIndex } = this.range.getRange()
    if (!~startIndex && !~endIndex) return
    const elementList = this.draw.getElementList()
    // 커서가 있는 단락의 시작 인덱스 찾기
    let paraStart = startIndex
    while (paraStart > 0 && elementList[paraStart]?.value !== ZERO) {
      paraStart--
    }
    if (elementList[paraStart]?.value === ZERO) paraStart++
    // TAB 1개 삽입 (단락 시작에)
    this.draw.spliceElementList(elementList, paraStart, 0, [
      { value: '\t', type: ElementType.TAB }
    ])
    // 커서는 삽입 위치보다 뒤에 있으면 +1 보정, 현재 위치 유지
    const newEnd = endIndex + 1
    const newStart = startIndex + 1
    this.range.replaceRange({ startIndex: newStart, endIndex: newEnd })
    this.draw.render({ curIndex: newEnd, isSubmitHistory: true })
  }

  public indentDecrease() {
    const isDisabled = this.draw.isReadonly() || this.draw.isDisabled()
    if (isDisabled) return
    const { startIndex, endIndex } = this.range.getRange()
    if (!~startIndex && !~endIndex) return
    ...
    if (elementList[paraStart]?.value === ZERO) paraStart++
    // 단락 시작이 TAB이면 제거
    if (elementList[paraStart]?.type === ElementType.TAB) {
      this.draw.spliceElementList(elementList, paraStart, 1)
      // 커서는 제거된 위치보다 뒤에 있으면 -1 보정, 현재 위치 유지
      const newEnd = Math.max(paraStart - 1, endIndex - 1)
      const newStart = Math.max(paraStart - 1, startIndex - 1)
      this.range.replaceRange({ startIndex: newStart, endIndex: newEnd })
      this.draw.render({ curIndex: newEnd, isSubmitHistory: true })
    }
  }
