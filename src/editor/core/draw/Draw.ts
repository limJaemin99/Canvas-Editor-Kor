...
export class Draw {
  ...
  public computeRowList(payload: IComputeRowListPayload) {
    ...
    for (let i = 0; i < elementList.length; i++) {
      ...
      // 实际可用宽度
      const indentOffset =
        curRow.elementList.length === 1 && element.indent
          ? element.indent * scale
          : curRow.elementList.length > 1 && curRow.elementList[0]?.indent
          ? curRow.elementList[0].indent * scale
          : 0
      const offsetX =
        curRow.offsetX ||
        (element.listId && listStyleMap.get(element.listId)) ||
        0
      const availableWidth = innerWidth - offsetX - indentOffset
      // 增加起始位置坐标偏移量
      const isStartElement = curRow.elementList.length === 1
      x += isStartElement ? offsetX + indentOffset : 0
